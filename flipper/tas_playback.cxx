...
public:
  ...
  {
    if (m_file)
      storage_file_free(m_file);
  }

  void close()
  {
    if (m_file)
      storage_file_close(m_file);
  }
  ...
  bool open(SdFs* from, const char* path, uint16_t mode)
  {
    ...
    return storage_file_open(m_file, path, FS_AccessMode((uint16_t)mode & 0xFF), FS_OpenMode((uint16_t)mode >> 8));
  }
...
private:
  ...
  void initFile()
  {
    ...
    if (!m_file)
      m_file = storage_file_alloc(m_fs->m_storage);
  }
...
{
  furi_hal_gpio_init(pin, GpioMode((uint16_t)mode & 0xFF), GpioPull((uint16_t)mode >> 8), GpioSpeedVeryHigh);
}
...
template<class T1, class T2> auto min(T1 x, T2 y) { return x < y ? x : y;}
...
#define FMT_JOY(byte) INV_IF(frameDat[byte] != 0), PLUS_MIN_ZERO(frameDat[byte]), UN_SIGN(frameDat[byte]), INV_IF(frameDat[byte] != 0)
  ...
  if (console == N64)
    snprintf(inputStr, sizeof(inputStr),
             ...
             FMT_JOY(2), FMT_JOY(3),
             FMT_BTN(0, 1), FMT_BTN(0, 0), FMT_BTN(0, 3), FMT_BTN(0, 2),
             FMT_BTN(0, 7), FMT_BTN(0, 6), FMT_BTN(0, 5), FMT_BTN(0, 4),
             FMT_BTN(1, 5), FMT_BTN(1, 4),
             FMT_BTN(1, 1), FMT_BTN(1, 0), FMT_BTN(1, 3), FMT_BTN(1, 2));
  ...
  char progStr[32];
  snprintf(progStr, sizeof(progStr), "%07lu/%07lu:%03i%%", curFrame, numFrames, (int)(progress * 100));
    ...
    do {
        if(p && strlen(p)) {
            ...
            storage_common_migrate(
                storage, EXT_PATH("tas"), STORAGE_APP_DATA_PATH_PREFIX);
            furi_record_close(RECORD_STORAGE);
        ...
        if (!openTAS(furi_string_get_cstr(file_path)))
          continue;
